-- Keep self-service booking ownership tied to auth.uid() while preserving editable contact fields.
...
as $$
declare
  normalized_email text := lower(trim(coalesce(new.guest_email, '')));
  normalized_phone text := trim(coalesce(new.guest_phone, ''));
  authenticated_guest_id uuid := auth.uid();
  authenticated_email text := lower(trim(coalesce(auth.jwt() ->> 'email', '')));
  canonical_guest_id uuid;
  authenticated_guest_organization_id uuid;
  lock_key bigint;
begin
  -- The self-service RPC supplies auth.uid() explicitly. Never replace that identity
  -- with a guest selected from editable contact data.
  if authenticated_guest_id is not null and new.primary_guest_id = authenticated_guest_id then
    select guest.organization_id
      into authenticated_guest_organization_id
    from public.guests guest
    where guest.id = authenticated_guest_id;

    if authenticated_guest_organization_id is not null and authenticated_guest_organization_id <> new.organization_id then
      raise exception using errcode = '42501', message = 'authenticated_guest_organization_mismatch';
    end if;

    insert into public.guests (id, first_name, last_name, email, phone, organization_id)
    values (
      ...
      coalesce(nullif(authenticated_email, ''), normalized_email),
      normalized_phone,
      new.organization_id
    )
  ...
  lock_key := hashtextextended(new.organization_id::text || ':' || normalized_email, 0);
  perform pg_advisory_xact_lock(lock_key);

  select guest.id
    into canonical_guest_id
  from public.guests guest
  where lower(trim(guest.email)) = normalized_email
    and guest.organization_id = new.organization_id
  order by guest.created_at asc nulls last, guest.id asc
  limit 1;

  if canonical_guest_id is not null then
    new.primary_guest_id := canonical_guest_id;
    return new;
  end if;
  ...
  returning id into canonical_guest_id;
...
begin
  ...
  if lower(coalesce(apartment_row.name, '')) ~ '(test|do[[:space:]-]*not[[:space:]-]*book|opero[[:space:]-]*ai[[:space:]-]*phase|dogfood|internal|qa|sample|mock)' then raise exception using errcode = '42501', message = 'apartment_unavailable'; end if;
  ...
  if to_regclass('public.notification_events') is not null then
    begin
      ...
      values (booking_row.organization_id, 'booking_created', 'booking', booking_row.id::text, booking_row.id::text, booking_row.apartment_id::text, jsonb_build_object('bookingId', booking_row.id, 'apartmentId', booking_row.apartment_id, 'guestName', booking_row.guest_name, 'guestEmail', booking_row.guest_email, 'checkIn', booking_row.check_in_date, 'checkOut', booking_row.check_out_date, 'totalAmount', booking_row.total_amount, 'currency', booking_row.currency, 'bookingStatus', 'pending', 'paymentStatus', 'unpaid', 'actionUrl', '/bookings/' || booking_row.id), 'public-booking-request:' || booking_row.id, auth.uid()) returning id into event_id;
      insert into public.notifications (organization_id, recipient_user_id, event_id, title, message, action_url)
      select booking_row.organization_id, member.user_id, event_id, 'Новый запрос на бронирование', trim(booking_row.guest_name) || ': ' || booking_row.check_in_date || ' - ' || booking_row.check_out_date, '/bookings/' || booking_row.id
      from public.organization_members member where member.organization_id = booking_row.organization_id and member.status = 'active' and lower(trim(member.role_code)) in ('owner', 'manager');
