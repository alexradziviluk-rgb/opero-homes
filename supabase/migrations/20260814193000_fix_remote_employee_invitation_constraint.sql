-- Align the employee invitation upsert with the remote primary-key contract.
create or replace function public.accept_employee_invitation(invite_token text)
returns table (invitation_id uuid, organization_id uuid, role_code text)
language plpgsql
security definer
...
begin
  ...
  select lower(trim(coalesce(u.email, ''))) into current_email
  from auth.users u where u.id = current_uid;
  if current_email is null or current_email = '' then raise exception 'AUTH_EMAIL_MISSING'; end if;
  ...
  if invitation_row.accepted_at is not null then raise exception 'INVITATION_ALREADY_ACCEPTED'; end if;
  if invitation_row.expires_at <= now() then raise exception 'INVITATION_EXPIRED'; end if;
  if lower(trim(invitation_row.email)) <> current_email then raise exception 'INVITATION_EMAIL_MISMATCH'; end if;

  insert into public.organization_members (organization_id, user_id, role_code, status, invited_by)
  values (invitation_row.organization_id, current_uid, invitation_row.role_code, 'active', invitation_row.invited_by)
  ...
  insert into public.profiles (id, first_name, last_name, email, phone, role, status, created_at, updated_at)
  values (
    current_uid,
  ...
  set accepted_at = now(), accepted_by_user_id = current_uid, delivery_status = 'accepted', updated_at = now()
  where id = invitation_row.id;

  return query select invitation_row.id, invitation_row.organization_id, invitation_row.role_code;
end;
$$;
