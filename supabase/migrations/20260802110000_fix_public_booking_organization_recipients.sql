-- Fix public booking notification recipients for the production organization schema.
-- Notifications are best-effort; the booking request remains successful if delivery fails.
  ...
  if to_regclass('public.notification_events') is not null then
    begin
      ...
      insert into public.notifications (
        organization_id, recipient_user_id, event_id, type, title, message,
        entity_type, entity_id, action_url
      )
      select booking_row.organization_id, member.user_id, event_id, 'booking',
      ...
      where member.organization_id = booking_row.organization_id
        ...
        and lower(member.role_code) in ('owner', 'manager')
      on conflict (organization_id, event_id, recipient_user_id) do nothing;
    exception when others then
      raise warning 'public booking notification delivery failed for booking %', booking_row.id;
    end;
  end if;
