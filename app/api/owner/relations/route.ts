...
type OwnerAccessRow = { guest_id: string | null; user_id: string | null; owner_public_number: string | null; status: string; created_at: string; owner_name: string | null; owner_email: string | null; owner_phone: string | null };
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ ok: true, data: ((data ?? []) as OwnerAccessRow[]).map((row) => { const [firstName = "", ...lastNameParts] = String(row.owner_name ?? "").split(/\s+/); return { guestId: row.guest_id, userId: row.user_id, ownerPublicNumber: row.owner_public_number ?? null, status: row.status, createdAt: row.created_at, firstName, lastName: lastNameParts.join(" "), email: row.owner_email ?? "", phone: row.owner_phone ?? null }; }) });
}
