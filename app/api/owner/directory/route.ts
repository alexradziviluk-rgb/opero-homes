...
type DirectoryOwner = { guest_id: string; user_id: string | null };
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ ok: true, data: data as DirectoryOwner[] | null ?? [] });
}
...
export async function POST(request: Request) {
  ...
  const supabase = await createSupabaseServerClient();
  if (!supabase) return errorResponse(500, "Supabase is not configured");
  const body = await request.json().catch(() => null) as { apartmentId?: string; guestId?: string } | null;
  if (!body?.apartmentId || !body.guestId) return errorResponse(400, "Укажите квартиру и клиента.");
  const { data, error } = await supabase.rpc("assign_registered_client_as_property_owner", { target_organization_id: auth.context.organization.id, target_apartment_id: body.apartmentId, target_guest_id: body.guestId });
  if (error) return errorResponse(error.message.includes("CLIENT_NOT_FOUND") ? 404 : 422, error.message);
  ...
  const [{ data: apartment }, { data: profile }] = await Promise.all([
    ...
    supabase.from("guests").select("first_name,last_name,email").eq("id", body.guestId).eq("organization_id", auth.context.organization.id).single(),
  ]);
