...
export async function GET(request: Request) {
  ...
  let query = supabase.from("availability_blocks").select("id,apartment_id,start_date,end_date,status,reason_code,private_note,created_by,created_at,updated_at").eq("organization_id", auth.context.organization.id).eq("status", "active").order("start_date", { ascending: true });
