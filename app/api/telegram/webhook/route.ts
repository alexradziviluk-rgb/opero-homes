import { NextResponse } from "next/server";
import { createSupabaseServiceRoleClient } from "@/lib/supabase/server";

const rateBuckets = new Map<string, { startedAt: number; count: number }>();

export async function POST(request: Request) {
  const expected = process.env.TELEGRAM_WEBHOOK_SECRET;
  if (!expected || request.headers.get("x-telegram-bot-api-secret-token") !== expected) return NextResponse.json({ ok: false, error: "Unauthorized" }, { status: 401 });
  if (request.headers.get("content-type")?.toLowerCase().split(";")[0] !== "application/json") return NextResponse.json({ ok: false, error: "Invalid request" }, { status: 415 });
  const contentLength = Number(request.headers.get("content-length") || "0");
  if (contentLength > 32_768) return NextResponse.json({ ok: false, error: "Payload too large" }, { status: 413 });
  const ip = request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "unknown";
  const now = Date.now();
  const bucket = rateBuckets.get(ip);
  if (bucket && now - bucket.startedAt < 60_000 && bucket.count >= 30) return NextResponse.json({ ok: false, error: "Rate limit exceeded" }, { status: 429 });
  if (!bucket || now - bucket.startedAt >= 60_000) rateBuckets.set(ip, { startedAt: now, count: 1 }); else bucket.count += 1;
  const raw = await request.text();
  if (raw.length > 32_768) return NextResponse.json({ ok: false, error: "Payload too large" }, { status: 413 });
  let payload: { update_id?: unknown; callback_query?: { data?: string; message?: { chat?: { id?: number }; message_id?: number } } };
  try {
    payload = JSON.parse(raw);
  } catch {
    return NextResponse.json({ ok: false, error: "Invalid request" }, { status: 400 });
  }
  const updateId = typeof payload.update_id === "number" && Number.isSafeInteger(payload.update_id) ? payload.update_id : null;
  if (updateId === null) return NextResponse.json({ ok: true });
  const callback = payload?.callback_query;
  const data = callback?.data ?? "";
  const supabase = createSupabaseServiceRoleClient(); if (!supabase) return NextResponse.json({ ok: false, error: "Service unavailable" }, { status: 503 });
  const { error: replayError } = await supabase.from("support_telegram_updates").insert({ update_id: updateId });
  if (replayError?.code === "23505") return NextResponse.json({ ok: true, duplicate: true });
  if (replayError) return NextResponse.json({ ok: false, error: "Service unavailable" }, { status: 503 });
  const match = data.match(/^support:(accept|resolve):([a-f0-9]{36})$/i);
  const chatId = callback?.message?.chat?.id ? String(callback.message.chat.id) : "";
  if (!match || !chatId) return NextResponse.json({ ok: true });
  const { data: ticket, error: ticketError } = await supabase.from("support_tickets").select("id,status,telegram_chat_id,public_number").eq("telegram_action_token", match[2]).maybeSingle();
  if (ticketError || !ticket) return NextResponse.json({ ok: true });
  if (ticket.telegram_chat_id !== chatId && process.env.TELEGRAM_MANAGER_CHAT_ID !== chatId) return NextResponse.json({ ok: false, error: "Forbidden chat" }, { status: 403 });
  const nextStatus = match[1] === "accept" ? "in_progress" : "resolved";
  if (ticket.status === nextStatus) return NextResponse.json({ ok: true, duplicate: true });
  const messageId = callback?.message?.message_id ?? null;
  const update = { status: nextStatus, telegram_message_id: messageId ? String(messageId) : null, resolved_at: nextStatus === "resolved" ? new Date().toISOString() : null };
  const { error: updateError } = await supabase.from("support_tickets").update(update).eq("id", ticket.id).in("status", ["open", "assigned", "in_progress", "waiting_for_client"]);
  if (updateError) return NextResponse.json({ ok: false, error: "Unable to update ticket" }, { status: 422 });
  await supabase.from("support_audit_log").insert({ ticket_id: ticket.id, actor_type: "telegram", action: `telegram_${nextStatus}`, safe_metadata: { message_id: messageId } });
  return NextResponse.json({ ok: true });
}
