...
type SearchOwner = { user_id: string | null; owner_public_number: string | null; owner_name: string; owner_email: string; owner_phone: string | null; apartment_count: number };
...
export default function ApartmentOwnersPage() {
  ...
  async function searchOwners(event: React.FormEvent) {
    ...
    try {
      ...
      else if (!result.data?.length) setError("Собственник с таким email не найден.");
      else setSearchResults((result.data ?? []).filter((owner: SearchOwner) => owner.user_id));
    } catch {
