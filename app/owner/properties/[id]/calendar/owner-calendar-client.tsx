...
const reasonOptions = [
  ["owner_stay", "Личное пребывание"],
  ["family_or_guests", "Родственники или гости"],
  ["renovation", "Ремонт"],
...
type Block = { id: string; start_date: string; end_date: string; reason_code: string; private_note: string | null; status: string };
...
export default function OwnerCalendarClient() {
  ...
  function resetForm() { setEditingId(null); setStartDate(""); setEndDate(""); setReasonCode("owner_stay"); setPrivateNote(""); }
  function editBlock(block: Block) { setEditingId(block.id); setStartDate(block.start_date); setEndDate(block.end_date); setReasonCode(block.reason_code); setPrivateNote(block.private_note ?? ""); setMessage(""); setError(""); }

  async function saveBlock(event: React.FormEvent) {
    event.preventDefault(); setSaving(true); setError(""); setMessage("");
    const response = await fetch(`/api/owner/properties/${apartmentId}/blocks`, { method: editingId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ blockId: editingId, startDate, endDate, reasonCode, privateNote }) });
    const result = await response.json();
    if (!response.ok || !result.ok) setError(result.error ?? "Не удалось сохранить блокировку");
    else { setMessage("Даты успешно заблокированы и больше недоступны для бронирования."); resetForm(); await load(); }
    setSaving(false);
  }
  ...
  return <main className="min-h-screen bg-slate-950 px-5 py-8 text-slate-100"><div className="mx-auto max-w-6xl"><Link href="/owner" className="text-sm text-cyan-300">Назад к квартирам</Link><div className="mt-6 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm uppercase tracking-[0.25em] text-cyan-300">Календарь</p><h1 className="mt-2 text-3xl font-semibold">Доступность квартиры</h1><p className="mt-2 text-sm text-slate-400">Занято бронированием, заблокировано собственником или свободно.</p></div><button type="button" onClick={() => { resetForm(); document.getElementById("block-form")?.scrollIntoView({ behavior: "smooth" }); }} className="rounded-xl bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950">Заблокировать даты</button></div>
    ...
    <section id="block-form" className="mt-10 rounded-2xl border border-white/10 bg-slate-900 p-5"><h2 className="text-xl font-semibold">{editingId ? "Изменить блокировку" : "Заблокировать даты"}</h2><form onSubmit={saveBlock} className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm text-slate-300">Дата начала<input required type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-white" /></label><label className="text-sm text-slate-300">Дата окончания<input required type="date" value={endDate} onChange={(event) => setEndDate(event.target.value)} className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-white" /></label><label className="text-sm text-slate-300">Причина<select value={reasonCode} onChange={(event) => setReasonCode(event.target.value)} className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-white">{reasonOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-sm text-slate-300">Комментарий<input value={privateNote} onChange={(event) => setPrivateNote(event.target.value)} placeholder="Необязательно" className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-white" /></label><div className="flex gap-2 sm:col-span-2"><button type="submit" disabled={saving} className="rounded-xl bg-cyan-400 px-4 py-2 font-semibold text-slate-950 disabled:opacity-50">{saving ? "Сохраняем..." : editingId ? "Сохранить изменения" : "Заблокировать"}</button>{editingId ? <button type="button" onClick={resetForm} className="rounded-xl border border-white/10 px-4 py-2">Отмена</button> : null}</div></form>{error ? <p className="mt-4 text-sm text-rose-300">{error}</p> : null}{message ? <p className="mt-4 text-sm text-emerald-300">{message}</p> : null}</section>
    <section className="mt-6 rounded-2xl border border-white/10 bg-slate-900 p-5"><h2 className="text-xl font-semibold">Мои блокировки</h2><div className="mt-4 space-y-3">{ownBlocks.length === 0 ? <p className="text-sm text-slate-400">Активных блокировок нет.</p> : ownBlocks.filter((block) => block.status === "active").map((block) => <div key={block.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/5 p-3"><div><p className="font-medium">{displayDate(block.start_date)} — {displayDate(block.end_date)}</p><p className="text-sm text-slate-400">{reasonOptions.find(([value]) => value === block.reason_code)?.[1] ?? block.reason_code}{block.private_note ? `: ${block.private_note}` : ""}</p></div><div className="flex gap-2"><button type="button" onClick={() => editBlock(block)} className="rounded-lg border border-white/10 px-3 py-1 text-sm">Изменить</button><button type="button" disabled={saving} onClick={() => void removeBlock(block.id)} className="rounded-lg border border-rose-400/30 px-3 py-1 text-sm text-rose-300">Удалить</button></div></div>)}</div></section>
  </div></main>;
}
