      ...
      <section className="relative bg-[#173b32] text-[#f5f3ee]">
        ...
        <div className="relative mx-auto max-w-7xl px-5 pb-20 pt-6 sm:px-8 lg:px-12">
          <nav className="flex items-center justify-between gap-4" aria-label="Main navigation">
            <Link href="/" className="text-lg font-bold tracking-tight">opero<span className="text-[#e2b25c]">.</span></Link>
            <div className="hidden items-center gap-8 text-sm text-[#d7e0d8] md:flex"><a href="#features">{text.nav[0]}</a><a href="#how">{text.nav[1]}</a><Link href="/pricing">{text.nav[2]}</Link></div>
            <div className="flex items-center gap-2 text-sm">
              <Link href={language === "ru" ? "/?lang=en" : "/"} className="rounded-full border border-white/20 px-3 py-2 text-[#f5f3ee]" aria-label="Switch language">{language.toUpperCase()}</Link>
          ...
          <div className="mt-4 grid gap-2 rounded-2xl border border-white/15 bg-white/10 p-4 text-sm md:hidden"><a href="#features">{text.nav[0]}</a><a href="#how">{text.nav[1]}</a><Link href="/pricing">{text.nav[2]}</Link><Link href="/login">{text.login}</Link></div>
...
export default function Home() {
  return <Suspense fallback={<main className="min-h-screen bg-[#173b32]" />}><LandingContent /></Suspense>;
}
