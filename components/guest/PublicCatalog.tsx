      ...
      <main className="mx-auto w-full max-w-7xl p-4 sm:p-6 lg:p-8">
        ...
        <section className="space-y-12">
          <section id="catalog-results" aria-labelledby="catalog-heading">
            ...
            {isLoading ? <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-6 text-slate-300">{copy.loading}</div> : publicApartments.length === 0 ? <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-6 text-slate-300">Пока нет опубликованных объектов.</div> : visibleApartments.length === 0 ? <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-6"><p className="text-slate-100">По вашему запросу ничего не найдено</p><p className="mt-2 text-sm text-slate-400">Попробуйте изменить город, район, даты или количество гостей.</p><button type="button" onClick={resetFilters} className="mt-4 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 hover:bg-white/10">{copy.reset}</button></div> : viewMode === "list" ? <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{visibleApartments.map((apartment) => <ApartmentCard key={apartment.id} apartment={apartment} bookings={bookings} compact language={language} />)}</div> : null}
          </section>
          ...
          <section id="find-another-property" aria-labelledby="search-heading">
            ...
            <div className="mb-6 rounded-3xl border border-white/10 bg-slate-900/80 p-4 sm:p-5">
              ...
              <div className="mt-2 grid gap-2 md:grid-cols-2 lg:grid-cols-4"><label><div className="text-xs text-slate-400">{copy.city}</div><select value={selectedCity} onChange={(event) => { setSelectedCity(event.target.value); setSelectedDistrict(""); }} className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-white outline-none"><option value="">{copy.allCities}</option>{cities.map((city) => <option key={city} value={city}>{city}</option>)}</select></label><label><div className="text-xs text-slate-400">{copy.district}</div><select value={selectedDistrict} onChange={(event) => setSelectedDistrict(event.target.value)} className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-white outline-none"><option value="">{copy.allDistricts}</option>{districts.map((district) => <option key={district} value={district}>{district}</option>)}</select></label><div><div className="text-xs text-slate-400">{copy.dates}</div><div className="mt-1 grid grid-cols-2 gap-2"><label className="text-[11px] text-slate-500">{copy.checkIn}<input type="date" aria-label={copy.checkIn} value={checkIn} onChange={(event) => setCheckIn(event.target.value)} className="mt-0.5 w-full rounded-xl border border-white/10 bg-white/5 px-2 py-1.5 text-sm text-white outline-none" /></label><label className="text-[11px] text-slate-500">{copy.checkOut}<input type="date" aria-label={copy.checkOut} value={checkOut} onChange={(event) => setCheckOut(event.target.value)} className="mt-0.5 w-full rounded-xl border border-white/10 bg-white/5 px-2 py-1.5 text-sm text-white outline-none" /></label></div></div><label><div className="text-xs text-slate-400">{copy.guests}</div><input type="number" min={1} value={guests} onChange={(event) => setGuests(event.target.value)} className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-white outline-none" /></label></div>
