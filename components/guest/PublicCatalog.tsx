        ...
        <section className="space-y-12">
          ...
          <section id="find-another-property" aria-labelledby="search-heading">
            ...
            <div className="mb-6 rounded-3xl border border-white/10 bg-slate-900/80 p-6">
              <div className="mt-4 grid gap-3 lg:grid-cols-4"><label className="lg:col-span-2"><div className="text-xs text-slate-400">{copy.address}</div><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") document.getElementById("catalog-results")?.scrollIntoView({ behavior: "smooth", block: "start" }); }} placeholder="Alanya, Аланья, Mahmutlar, Ataturk, 305" className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none" /></label><label><div className="text-xs text-slate-400">{copy.sort}</div><select value={sortMode} onChange={(event) => setSortMode(event.target.value as SortMode)} className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none"><option value="recommended">{copy.recommended}</option><option value="price_asc">{copy.cheapest}</option><option value="price_desc">{copy.expensive}</option><option value="capacity_desc">{copy.capacity}</option></select></label><div className="flex items-end gap-2"><button type="button" onClick={() => document.getElementById("catalog-results")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="w-full rounded-xl bg-cyan-400 px-3 py-2 text-sm font-semibold text-slate-950 hover:bg-cyan-300">{copy.apply}</button><button type="button" onClick={resetFilters} className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 hover:bg-white/10">{copy.reset}</button></div></div>
