...
test("Telegram identities allow one active user and one active chat", () => {
  ...
  const fix = readFileSync("supabase/migrations/20260809160000_fix_telegram_link_token_rpc_ambiguity.sql", "utf8");
  ...
  expect(fix).toContain("on conflict on constraint support_telegram_bindings_pkey");
  expect(fix).toContain("link_token.revoked_at is null");
});
