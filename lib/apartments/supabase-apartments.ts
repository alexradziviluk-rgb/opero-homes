...
function mapApartmentRow(row: Record<string, unknown>, photos: ApartmentPhoto[]): Apartment {
  ...
  const normalized = normalizeApartment({
    ...
    country: safeString(row.country) || "Турция",
    ...
    houseRules: typeof row.house_rules === "object" && row.house_rules !== null ? row.house_rules as Apartment["houseRules"] : undefined,
  });
...
export async function saveApartmentToSupabase(apartment: Apartment): Promise<Apartment> {
  ...
  const payload = {
    ...
    unit_number: apartment.unitNumber?.trim() || null,
    type: apartment.type,
    ...
    beds: toNullableInteger(apartment.beds),
    ...
    deposit: toNullableNumber(apartment.deposit) ?? 0,
    cleaning_fee: toNullableNumber(apartment.cleaningFee) ?? 0,
    ...
    house_rules: apartment.houseRules ?? null,
